...
void listen_inputs(){
  ...
  if(inputs_prev[0] > inputs_now[0]){
    if(antirrebote(IN_A_start)){
      count_cars[SA]++;
      Serial.println("Entro un vehiculo en calle A");
    }
  }
  if(inputs_prev[1] > inputs_now[1]){
    if(antirrebote(IN_A_finish)){
      count_cars[SA]--;
      Serial.println("Salió un vehiculo en calle A");
  ...
  if(inputs_prev[2] > inputs_now[2]){
    if(antirrebote(IN_B_start)){
      count_cars[SB]++;
      Serial.println("Entro un vehiculo en calle B");
    } 
  }
  if(inputs_prev[3] > inputs_now[3]){
    if(antirrebote(IN_B_finish)){
      count_cars[SB]--;
      Serial.println("Salió un vehiculo en calle B");
